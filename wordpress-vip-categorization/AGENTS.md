...
- Format code when needed: `black src tests`
...
- Every new CLI command must ship with a corresponding integration test since CLI code is excluded from unit coverage.
...
- Follow Conventional Commits (`type(scope): description`) for all commit messages.
- Run the full pre-commit hook suite locally; hooks enforce the required quality gates (`black --check`, `ruff check`, `mypy src`, `pytest --cov-fail-under=80`).
