---
description: Host and expose a web application in local sandbox with public URL
argument-hint: [sandbox_id] [port: 5173] [backend: tmux]
---

# Host Application

Expose and test a web application running in a local sandbox by starting the server, retrieving the public URL, and verifying it's accessible.
...
3. **Get Public URL** - Use `sbx sandbox get-host SANDBOX_ID --port PORT --backend BACKEND` to retrieve the authoritative public URL
...
The sandbox will automatically terminate based on the configured timeout.
```
